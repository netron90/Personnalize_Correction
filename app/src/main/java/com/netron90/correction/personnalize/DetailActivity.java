...
//    private DeleteDocumentSend deleteDocumentSend;
    ...
    private final int MONTANT_CORRECTION = 200, MONTANT_MISE_PAGE = 50;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        correctionFauteFactureTv.setText(String.valueOf(userDoc.pageNumber * MONTANT_CORRECTION)+" f CFA");
                    ...
                    if(userPhone.equals("No PhoneNumber") || userPhone.equals(""))
                    {
                        ...
                        alertDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                ...
                                String uploadingMessage = "Veuillez patienter...";
                                pd = new ProgressDialog(DetailActivity.this);
                                pd.setTitle("Envoie du document");
    ...
    public class GetDiapositive extends AsyncTask<Void, Void, List<DiapositiveFormat>>
    {
        ...
        protected void onPostExecute(List<DiapositiveFormat> diapositiveFormats) {
            ...
            else
            {
                ...
                MONTANT_TO_SEND = userDoc.pageNumber * MONTANT_MISE_PAGE;
                correctionMiseEnFormeFactureTv.setText(String.valueOf(userDoc.pageNumber * MONTANT_MISE_PAGE) + " f CFA");
            }

            if(!userDoc.powerPoint && !userDoc.miseEnForme)
            {
                MONTANT_TO_SEND = MONTANT_CORRECTION;
                factureTotalTv.setText(String.valueOf(MONTANT_CORRECTION * userDoc.pageNumber)+" f CFA");
            }
            else if(!userDoc.powerPoint && userDoc.miseEnForme)
            {
                MONTANT_TO_SEND = (MONTANT_MISE_PAGE * userDoc.pageNumber) + (userDoc.pageNumber * MONTANT_CORRECTION);
                factureTotalTv.setText(String.valueOf((MONTANT_MISE_PAGE * userDoc.pageNumber) + (userDoc.pageNumber * MONTANT_CORRECTION))+" f CFA");
            }
            else if(userDoc.powerPoint && !userDoc.miseEnForme)
            {
                if(getListOnStart.size() > 25)
                {
                    MONTANT_TO_SEND = (MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 200);
                    factureTotalTv.setText(String.valueOf((MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 200) )+" f CFA");
                }
                else {
                    MONTANT_TO_SEND = (MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 100);
                    factureTotalTv.setText(String.valueOf((MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 100) )+" f CFA");
                }
            }
            else if(userDoc.powerPoint && userDoc.miseEnForme){
                if(getListOnStart.size() > 25)
                {
                    MONTANT_TO_SEND = (MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 200) + (userDoc.pageNumber * MONTANT_MISE_PAGE);
                    factureTotalTv.setText(String.valueOf((MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 200) + (userDoc.pageNumber * MONTANT_MISE_PAGE))+" f CFA");
                }
                else {
                    MONTANT_TO_SEND = MONTANT_CORRECTION + (getListOnStart.size() * 100) + (userDoc.pageNumber * MONTANT_MISE_PAGE);
                    factureTotalTv.setText(String.valueOf((MONTANT_CORRECTION * userDoc.pageNumber) + (getListOnStart.size() * 100) + userDoc.pageNumber * MONTANT_MISE_PAGE)+" f CFA");
                }
            }else{
        ...
        try {
            ...
            mAsyncHttpClient.post("http://mighty-refuge-23480.herokuapp.com/memoire_api", mRequestParams, new JsonHttpResponseHandler() {
                ...
                public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                    ...
                    Log.d("Serveur responseSuccess", "Status Code: " + statusCode + " Response body: " + response);
                    ...
                    sendMailClient.setTimeout(30000);
                    sendMailClient.setResponseTimeout(30000);
                    sendMailClient.setMaxRetriesAndTimeout(3, 30000);
                    sendMailClient.get("http://mighty-refuge-23480.herokuapp.com/send_mail_api", sendMailParamsClient, new JsonHttpResponseHandler(){
                        ...
                        public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                            ...
                            AlertDialog.Builder alertDialogError = new AlertDialog.Builder(getApplicationContext());
                            alertDialogError.setTitle("Erreur de connexion au serveur");
                            View v = View.inflate(getApplicationContext(), R.layout.error_connexion_dialog, null);
                            alertDialogError.setView(v);
                            alertDialogError.setMessage("Le serveur ne répond pas. Assurez-vous que votre connexion est active et réessayer");
                            alertDialogError.setPositiveButton("Réessayer", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    sendDocumentToTeam();
                                }
                ...
                public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
                    ...
                    Log.d("Failed: ", ""+statusCode);
                    Log.d("Error : ", "" + throwable);
                    Log.d("Serveur response", "Status Code: " + statusCode + " errorResponse body object: " + errorResponse + " throwable: " + throwable);
                    pd.dismiss();
                    //Toast.makeText(getApplicationContext(), "Erreur de connextion: " + String.valueOf(throwable instanceof ConnectTimeoutException),Toast.LENGTH_LONG).show();
                    AlertDialog.Builder alertDialogError = new AlertDialog.Builder(getApplicationContext());
                    alertDialogError.setTitle("Erreur de connexion au serveur");
                ...
                public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONArray errorResponse) {
                    ...
                    Log.d("Failed: ", ""+statusCode);
                    Log.d("Error : ", "" + throwable);
                    Log.d("Serveur response", "Status Code: " + statusCode + " errorResponse body array: " + errorResponse + " throwable: " + throwable);
                    pd.dismiss();
                    //Toast.makeText(getApplicationContext(), "Erreur de connextion: " + String.valueOf(throwable instanceof ConnectTimeoutException),Toast.LENGTH_LONG).show();
                    AlertDialog.Builder alertDialogError = new AlertDialog.Builder(getApplicationContext());
                    alertDialogError.setTitle("Erreur de connexion au serveur");
